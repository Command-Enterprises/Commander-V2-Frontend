// Branded with Commander, but entirely passively modded from https://github.com/binary-person/womginx/blob/master/public/wombat-handler.js
(function () {
    ...
    if (window && window._WBWombat && !window._wb_js_inited && !window._wb_wombat) {
        ...
        window.history._commander_replaceState = window.history.replaceState;
        window.history.replaceState = function (stateObj, title, url) {
            if (window.location.pathname.startsWith("/main/https://www.google.com")) {
                url = "";
            }
            return this._commander_replaceState(stateObj, title, url);
        };

        // auto-merge slashes if server merges them with redirects (which break non-GET requests such as POST)
        window.XMLHttpRequest.prototype._commander_open = window.XMLHttpRequest.prototype.open;
        window.XMLHttpRequest.prototype.open = function (method, url, async, username, password) {
            return this._commander_open(method, mergeDoubleSlash(url), async, username, password);
        };
        window.XMLHttpRequest.prototype.url
        window._commander_fetch = window.fetch;
        window.fetch = function (input, init) {
            if (typeof input === 'string') {
                return window._commander_fetch(mergeDoubleSlash(input), init);
            }
            var request = new Request(input, init);
            return _commander_fetch(mergeDoubleSlash(input.url), request);
        };

        // add websocket origin support and auto-merge slashes
        window._commander_WebSocket = window.WebSocket;
        window.WebSocket = function (url, protocols) {
            url = url + '?commander_ws_origin_header=' + dest_scheme + '://' + dest_host;
            return new window._commander_WebSocket(mergeDoubleSlash(url), protocols);
        };
        ...
        window._commander_Blob = window.Blob;
        window.Blob = function (data, options = {}) {
            return new window._commander_Blob(data, options);
        };
        ...
        window._wb_wombat._commander_rewriteWorker = window._wb_wombat.rewriteWorker;
        window._wb_wombat.rewriteWorker = function (workerUrl) {
            ...
            return this._commander_rewriteWorker(workerUrl);
        };
        ...
        window._wb_wombat._commander_rewriteElem = window._wb_wombat.rewriteElem;
        window._wb_wombat.rewriteElem = function (elem) {
            ...
            return this._commander_rewriteElem(elem);
        };
